...
//
// Those break things under GNUstep.
//
#ifdef MACOSX
- (float) height
{
  if ([_controller isKindOfClass: [MailboxInspectorPanelController class]])
    {
      return [[_controller textView] frame].size.width-10;
    }
  
  return [[_controller mailHeaderCell] height];
}

- (float) width
{
  return THREAD_ARCS_CELL_WIDTH;
}

- (NSSize) cellSize
{
  return NSMakeSize(THREAD_ARCS_CELL_WIDTH , [self height]);
}
#endif
  ...
  if (_uses_inspector)
    {
      ...
      theFrame.size.width -= 9;
      theFrame.size.height -= 9;
    }
  else
    {
      aCell = [_controller mailHeaderCell];
      theFrame.size.height = [aCell height];
		   ...
		   inView: (NSView *) theView
{
  ...
  int i, j, origin, height, count;
  float scrollview_width;
  BOOL flip;
