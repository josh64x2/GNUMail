...
- (IBAction) deleteMessage: (id) sender
{
  ...
  if ([_folder count] == 0 || [dataView numberOfSelectedRows] == 0)
      ...
      for (i = 0; i < [selectedRows count]; i++)
	{
	  ...
	  theMessage = [_allVisibleMessages objectAtIndex: [aRow intValue]];
  ...
  for (i = 0; i < count; i++)
    {
      ...
      aMessage = [_allVisibleMessages objectAtIndex: [[rows objectAtIndex: i] intValue]];
...
- (NSArray *) selectedMessages
{
  if ([dataView numberOfSelectedRows] == 0)
	...
	{
	  aMessage = [_allVisibleMessages objectAtIndex: [aRow intValue]];
...
- (void) updateStatusLabel
{
  ...
  if (numberOfSelectedRows > 0)
	...
	{
	  aMessage = [_allVisibleMessages objectAtIndex: [anObject intValue]];
...
- (void) _showMessage: (id) sender
{
  ...
  // Under GNUstep, the value never gets below 3, that's why
  // we picked the value 4.
  if ([textScrollView frame].size.height < 4)
    {
      return;
    }
